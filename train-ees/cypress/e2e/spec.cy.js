...
describe("Check new card inputs", () => {
  it("passes", () => {
    ...
    cy.get('.cardHeader > input:first-child').click().type('John');
    cy.get('.cardHeader > input:nth-child(2)').click().type('Doe');
    cy.get('.cardHeader > input:last-child').click().type('Cleaner');
    cy.get('.linkedin > input').click().type('john-doe');
    cy.get('.github > input').click().type('johndoe');
    cy.get('.phone > input').click().type('+44 7000 0000');
    cy.get('.email > input').click().type('john@doe.com');
    cy.window().then(function(promptelement){
      cy.stub(promptelement, "prompt").returns('');
      cy.get('.profilePicture > img').click();
    });
    cy.get('.enterPassword > input').click().type('pass');
    cy.get('.createBtn > button').click();
